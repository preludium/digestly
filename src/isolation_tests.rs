...
async fn call(app: &axum::Router, method: &str, uri: &str, body: Option<Value>, cookie: Option<&str>) -> Resp {
    ...
    Resp { status, body, cookie: set_cookie }
}

/// Like `call` but returns the raw response body as text (for non-JSON endpoints, e.g. OPML export).
async fn call_text(app: &axum::Router, method: &str, uri: &str, cookie: Option<&str>) -> (StatusCode, String) {
    let mut builder = Request::builder().method(method).uri(uri);
    if let Some(c) = cookie {
        builder = builder.header(header::COOKIE, c);
    }
...
async fn login(app: &axum::Router, username: &str, password: &str) -> Resp {
    call(app, "POST", "/api/auth/login", Some(json!({ "username": username, "password": password })), None).await
}

async fn register(app: &axum::Router, username: &str, password: &str) -> Resp {
    call(app, "POST", "/api/auth/register", Some(json!({ "username": username, "password": password })), None).await
}
...
async fn admin_can_log_in_and_bad_password_is_rejected() {
    ...
    assert_eq!(unknown.status, StatusCode::UNAUTHORIZED, "no username enumeration");
}
...
async fn users_cannot_read_each_others_rows() {
    ...
    let ids = |v: &Value| -> Vec<i64> {
        v.as_array().unwrap().iter().map(|c| c["id"].as_i64().unwrap()).collect()
    };
...
async fn builtin_admin_and_last_admin_are_protected() {
    ...
    let del = call(&app, "DELETE", &format!("/api/admin/users/{admin_id}"), None, Some(&admin_c)).await;
...
async fn change_password_requires_current() {
    ...
    assert_eq!(login(&app, "alice", "newpassword1").await.status, StatusCode::OK);
    assert_eq!(login(&app, "alice", "password123").await.status, StatusCode::UNAUTHORIZED);
}
...
async fn deleting_category_reassigns_feeds_to_other_and_other_is_protected() {
    ...
    let del = call(&app, "DELETE", &format!("/api/categories/{ai}"), None, Some(&alice_c)).await;
    assert_eq!(del.status, StatusCode::OK);

    let feeds = call(&app, "GET", "/api/feeds", None, Some(&alice_c)).await;
    let f = &feeds.body.as_array().unwrap()[0];
    assert_eq!(f["category_id"].as_i64().unwrap(), other, "feed moved to Other");
    assert_eq!(f["category_name"], "Other");

    // Other itself cannot be deleted.
    let del_other = call(&app, "DELETE", &format!("/api/categories/{other}"), None, Some(&alice_c)).await;
...
async fn feeds_are_per_user_scoped_over_a_shared_catalog() {
    ...
    assert_ne!(a_sub.body["id"], b_sub.body["id"], "distinct per-user subscriptions");
    ...
    let del = call(&app, "DELETE", &format!("/api/feeds/{alice_sub_id}"), None, Some(&bob_c)).await;
...
fn item_ids(body: &Value) -> Vec<i64> {
    body["items"].as_array().unwrap().iter().map(|i| i["id"].as_i64().unwrap()).collect()
}
...
async fn items_are_per_user_over_shared_content_with_independent_state() {
    ...
    let item = insert_item(&pool, feed_id, "g1", "Hello World", "some body text", "2021-06-15 12:00:00", None, None, Some(120)).await;
    ...
    assert_eq!(call(&app, "GET", "/api/items", None, Some(&bob_c)).await.body["total_count"].as_i64().unwrap(), 1);

    // Alice stars + reads; no `item_states` row was pre-created, this upserts one.
    let star = call(&app, "POST", &format!("/api/items/{item}/star"), Some(json!({ "value": true })), Some(&alice_c)).await;
    assert_eq!(star.status, StatusCode::OK);
    assert_eq!(star.body["is_starred"], true);
    let read = call(&app, "POST", &format!("/api/items/{item}/read"), Some(json!({ "value": true })), Some(&alice_c)).await;
    ...
    let a_view = call(&app, "GET", &format!("/api/items/{item}"), None, Some(&alice_c)).await;
    assert_eq!(a_view.body["is_starred"], true);
    assert_eq!(a_view.body["is_read"], true);
    let b_view = call(&app, "GET", &format!("/api/items/{item}"), None, Some(&bob_c)).await;
    ...
    assert_eq!(call(&app, "GET", "/api/items?status=starred", None, Some(&alice_c)).await.body["total_count"], 1);
    assert_eq!(call(&app, "GET", "/api/items?status=starred", None, Some(&bob_c)).await.body["total_count"], 0);

    // Carol doesn't subscribe → she can neither read nor mutate the item (404, not another feed's data).
    assert_eq!(call(&app, "GET", &format!("/api/items/{item}"), None, Some(&carol_c)).await.status, StatusCode::NOT_FOUND);
    assert_eq!(
        call(&app, "POST", &format!("/api/items/{item}/star"), Some(json!({ "value": true })), Some(&carol_c)).await.status,
        StatusCode::NOT_FOUND,
    );
    assert_eq!(call(&app, "GET", "/api/items", None, Some(&carol_c)).await.body["total_count"].as_i64().unwrap(), 0);

    // Toggle (no body value) flips Alice's read back to unread.
    let toggled = call(&app, "POST", &format!("/api/items/{item}/read"), None, Some(&alice_c)).await;
...
async fn item_facets_sorts_and_search_work_end_to_end() {
    ...
    let b = insert_item(&pool, feed_id, "g2", "Gardening tips", "grow tomatoes", "2021-06-11 10:00:00", None, None, Some(120)).await;
    let cc = insert_item(&pool, feed_id, "g3", "Rust web servers", "axum guide", "2021-06-12 10:00:00", Some(5), Some(50), Some(300)).await;
    ...
    assert_eq!(item_ids(&call(&app, "GET", "/api/items?sort=new", None, Some(&c)).await.body), vec![cc, b, a]);
    assert_eq!(item_ids(&call(&app, "GET", "/api/items?sort=old", None, Some(&c)).await.body), vec![a, b, cc]);
    ...
    assert!(call(&app, "GET", "/api/items?q=%21%40%23", None, Some(&c)).await.status.is_success());

    // Pagination: page_size=2 over 3 items → 2 pages.
    let p1 = call(&app, "GET", "/api/items?page_size=2&page=1&sort=old", None, Some(&c)).await;
    ...
    let p2 = call(&app, "GET", "/api/items?page_size=2&page=2&sort=old", None, Some(&c)).await;
    assert_eq!(item_ids(&p2.body), vec![cc]);

    // type=video → none (reading subscription); type=reading → all three.
    assert_eq!(call(&app, "GET", "/api/items?type=video", None, Some(&c)).await.body["total_count"].as_i64().unwrap(), 0);
    assert_eq!(call(&app, "GET", "/api/items?type=reading", None, Some(&c)).await.body["total_count"].as_i64().unwrap(), 3);
}
...
async fn category_counts_reflect_active_facets() {
    ...
    insert_item(&pool, ai_feed, "a1", "one", "x", "2021-06-10 10:00:00", None, None, None).await;
    insert_item(&pool, ai_feed, "a2", "two", "x", "2021-06-10 10:00:00", None, None, None).await;
    let se_item = insert_item(&pool, se_feed, "s1", "three", "x", "2021-06-10 10:00:00", None, None, None).await;

    let count_for = |body: &Value, cid: i64| -> i64 {
        body["categories"].as_array().unwrap().iter()
            .find(|x| x["category_id"].as_i64() == Some(cid))
            .unwrap()["count"].as_i64().unwrap()
    };
    ...
    call(&app, "POST", &format!("/api/items/{se_item}/read"), Some(json!({ "value": true })), Some(&c)).await;
    let unread = call(&app, "GET", "/api/categories/counts?status=unread", None, Some(&c)).await;
...
async fn ai_endpoints_require_admin() {
    ...
    for (method, uri, body) in [
        ...
        ("POST", "/api/ai/providers", Some(json!({ "name": "x", "provider_type": "custom", "api_style": "openai", "base_url": "https://api.example.com/v1", "model": "m", "key": "secret" }))),
    ] {
        let user = call(&app, method, uri, body.clone(), Some(&alice_c)).await;
        assert_eq!(user.status, StatusCode::FORBIDDEN, "non-admin must get 403 on {method} {uri}");
        let anon = call(&app, method, uri, body, None).await;
        assert_eq!(anon.status, StatusCode::UNAUTHORIZED, "anon must get 401 on {method} {uri}");
    }
}
...
async fn ai_ssrf_guard_allows_ollama_but_blocks_private_custom() {
    ...
    let ollama = call(
        ...
        "POST",
        "/api/ai/providers",
        Some(json!({ "name": "Ollama", "provider_type": "ollama", "api_style": "openai",
                     "base_url": "http://localhost:11434/v1", "model": "llama3.2" })),
        Some(&admin_c),
    )
    .await;
    assert_eq!(ollama.status, StatusCode::OK, "localhost Ollama passes SSRF");
    ...
    let private = call(
        ...
        "POST",
        "/api/ai/providers",
        Some(json!({ "name": "Evil", "provider_type": "custom", "api_style": "openai",
                     "base_url": "http://127.0.0.1:9000/v1", "model": "m", "key": "k" })),
        Some(&admin_c),
    )
    .await;
    assert_eq!(private.status, StatusCode::BAD_REQUEST, "private custom URL blocked by default");

    // Turn allow-private on → the same custom URL is now accepted.
    call(&app, "PUT", "/api/admin/settings", Some(json!({ "allow_registration": true })), Some(&admin_c)).await;
    ...
    let allowed = call(
        ...
        "POST",
        "/api/ai/providers",
        Some(json!({ "name": "LAN", "provider_type": "custom", "api_style": "openai",
                     "base_url": "http://127.0.0.1:9000/v1", "model": "m", "key": "k" })),
        Some(&admin_c),
    )
    .await;
    assert_eq!(allowed.status, StatusCode::OK, "allow-private lets a private custom URL through");
}
...
async fn summarize_reuses_shared_cache_across_users() {
    ...
    let a = call(&app, "POST", &format!("/api/items/{item}/summarize"), None, Some(&alice_c)).await;
    ...
    let b = call(&app, "POST", &format!("/api/items/{item}/summarize"), None, Some(&bob_c)).await;
...
async fn summarize_without_provider_is_a_clear_error() {
    ...
    let r = call(&app, "POST", &format!("/api/items/{item}/summarize"), None, Some(&alice_c)).await;
    ...
    let bob_c = register(&app, "bob", "password123").await.cookie.unwrap();
    let other = call(&app, "POST", &format!("/api/items/{item}/summarize"), None, Some(&bob_c)).await;
...
async fn feed_health_transition_is_detected_once_per_episode() {
    use crate::ingest::fetch::{Fetched, FetchError};
    ...
    let t1 = store::record_failure(&pool, feed_id, &FetchError::Transient("boom".into())).await.unwrap();
    ...
    let t2 = store::record_failure(&pool, feed_id, &FetchError::Transient("boom".into())).await.unwrap();
    assert!(!t2, "still-failing must not re-notify every poll");

    // Recovery resets health; the next failure is a fresh transition again.
    let fetched = Fetched { body: Vec::new(), etag: None, last_modified: None, permanent_url: None };
    store::record_success(&pool, feed_id, &fetched, 3600).await.unwrap();
    let t3 = store::record_failure(&pool, feed_id, &FetchError::Transient("boom".into())).await.unwrap();
...
async fn feed_health_recipients_are_deduped_and_scoped() {
    ...
    assert_eq!(recipients, expected, "exactly the two subscribers with health-on + a channel, de-duped");
}
...
async fn digest_run_is_admin_only_and_archived_per_user_with_raw_fallback() {
    ...
    let detail = call(&app, "GET", &format!("/api/digest/{id}"), None, Some(&alice_c)).await;
    ...
    let bob_c = register(&app, "bob", "password123").await.cookie.unwrap();
    let cross = call(&app, "GET", &format!("/api/digest/{id}"), None, Some(&bob_c)).await;
...
async fn digest_run_accepts_a_custom_lookback_override() {
    ...
    let ten_days_ago = (chrono::Utc::now() - chrono::Duration::days(10)).format("%Y-%m-%d %H:%M:%S").to_string();
    insert_item(&pool, feed_id, "old", "Old news", "old text", &ten_days_ago, None, None, None).await;
    ...
    assert_eq!(list1.body.as_array().unwrap()[0]["item_count"].as_i64().unwrap(), 1);
    ...
    assert_eq!(list2.body.as_array().unwrap()[0]["item_count"].as_i64().unwrap(), 2, "30-day override should include the 10-day-old item");
    ...
    assert_eq!(cfg.body["lookback_days"].as_i64().unwrap(), 1, "manual override must not persist to app_settings");
}
...
async fn notifications_config_never_returns_the_token() {
    ...
    .await;
    assert_eq!(put.status, StatusCode::OK);
    assert!(!put.body.to_string().contains(secret), "PUT response must not echo the token");
...
async fn per_user_settings_round_trip_and_validate() {
    ...
    let ok = call(&app, "PUT", "/api/settings", Some(json!({ "timezone": "Europe/Warsaw", "page_size": 25, "onboarded": true })), Some(&c)).await;
    assert_eq!(ok.status, StatusCode::OK);
    ...
    assert_eq!(call(&app, "PUT", "/api/settings", Some(json!({ "timezone": "Mars/Olympus" })), Some(&c)).await.status, StatusCode::BAD_REQUEST);
    assert_eq!(call(&app, "PUT", "/api/settings", Some(json!({ "sort": "sideways" })), Some(&c)).await.status, StatusCode::BAD_REQUEST);
}
...
async fn opml_export_import_round_trips_losslessly_via_api() {
    ...
    let bob_c = register(&app, "bob", "password123").await.cookie.unwrap();
    let preview = call(&app, "POST", "/api/opml/import", Some(json!({ "opml": xml })), Some(&bob_c)).await;
    ...
    let items: Vec<Value> = entries
        ...
        .collect();
    let confirm = call(&app, "POST", "/api/opml/import", Some(json!({ "items": items })), Some(&bob_c)).await;
    ...
    let again = call(&app, "POST", "/api/opml/import", Some(json!({ "items": items })), Some(&bob_c)).await;
...
async fn retention_purge_removes_old_but_keeps_starred_forever() {
    ...
    let old_plain = insert_item(&pool, feed_id, "op", "Old Plain", "x", "2000-01-02 00:00:00", None, None, None).await;
    let recent = insert_recent_item(&pool, feed_id, "rc", "Recent").await;

    // Alice stars the old item → it must survive retention forever.
    call(&app, "POST", &format!("/api/items/{old_starred}/star"), Some(json!({ "value": true })), Some(&alice_c)).await;
    ...
    let exists = |id: i64| {
        let pool = pool.clone();
        async move {
            sqlx::query("SELECT 1 FROM items WHERE id = ?").bind(id).fetch_optional(&pool).await.unwrap().is_some()
        }
    };
...
async fn retention_purge_endpoint_is_admin_only_and_deletes_now() {
    ...
    let denied = call(&app, "POST", "/api/admin/retention/purge", None, Some(&alice_c)).await;
    assert_eq!(denied.status, StatusCode::FORBIDDEN);

    // No retention limit set yet → purge is a safe no-op.
    let noop = call(&app, "POST", "/api/admin/retention/purge", None, Some(&admin_c)).await;
    ...
    let put = call(
        ...
        Some(json!({
            ...
            "default_interval_secs": 900, "allow_private": false,
    ...
    let run = call(&app, "POST", "/api/admin/retention/purge", None, Some(&admin_c)).await;
    assert_eq!(run.status, StatusCode::OK);
    assert_eq!(run.body["removed"], 1, "only the old, non-starred item is purged");

    let exists = |id: i64| {
        let pool = pool.clone();
        async move { sqlx::query("SELECT 1 FROM items WHERE id = ?").bind(id).fetch_optional(&pool).await.unwrap().is_some() }
    };
    assert!(exists(old_starred).await, "starred item kept forever");
    assert!(!exists(old_plain).await, "old non-starred item purged");
    assert!(exists(recent).await, "recent item kept");
...
async fn bundled_fixture_feeds_parse_offline_and_sanitize() {
    ...
    for (file, kind, expected) in [
        ...
        let parsed = parse::parse_feed(&bytes, "https://fixtures.example/feed", kind, &cfg, Utc::now()).unwrap();
    ...
    let parsed = parse::parse_feed(&rss, "https://fixtures.example/feed", FeedKind::Rss, &cfg, Utc::now()).unwrap();
    let html = parsed.items.iter().filter_map(|i| i.content_html.clone()).collect::<String>();
    assert!(!html.contains("<script"), "scripts stripped");
    assert!(!html.to_lowercase().contains("javascript:"), "javascript: URLs stripped");
}
...
async fn passkey_endpoints_are_wired_scoped_and_report_enabled() {
    ...
    let opts = call(&app, "POST", "/api/passkeys/register/options", None, Some(&alice_c)).await;
    assert_eq!(opts.status, StatusCode::OK);
    assert!(opts.body["ceremony_id"].as_str().is_some());
    assert!(opts.body["options"]["publicKey"]["challenge"].as_str().is_some());

    // Passwordless login for an account with no passkeys is a clear, non-crashing error.
    let no_pk = call(&app, "POST", "/api/auth/passkey/login/options", Some(json!({ "username": "bob" })), None).await;
    ...
    assert_eq!(bob_list.body.as_array().unwrap().len(), 0, "bob never sees alice's passkey");

    // Bob cannot rename or delete alice's passkey.
    let steal_rename = call(&app, "PATCH", &format!("/api/passkeys/{pk_id}"), Some(json!({ "name": "hax" })), Some(&bob_c)).await;
    assert_eq!(steal_rename.status, StatusCode::NOT_FOUND);
    let steal_del = call(&app, "DELETE", &format!("/api/passkeys/{pk_id}"), None, Some(&bob_c)).await;
...
async fn last_sign_in_method_cannot_be_deleted() {
    ...
    let del_ok = call(&app, "DELETE", &format!("/api/passkeys/{pk_id}"), None, Some(&alice_c)).await;
    assert_eq!(del_ok.status, StatusCode::OK, "deleting a passkey is allowed while a password exists");
    ...
    sqlx::query("UPDATE users SET password_hash = NULL WHERE id = ?").bind(alice_id).execute(&pool).await.unwrap();

    let blocked = call(&app, "DELETE", &format!("/api/passkeys/{pk_id2}"), None, Some(&alice_c)).await;
    assert_eq!(blocked.status, StatusCode::BAD_REQUEST, "cannot delete the only sign-in method");
    // It's still there.
...
async fn passkey_end_to_end_register_login_and_sign_count_regression() {
    ...
    let ccr: CreationChallengeResponse = serde_json::from_value(opts.body["options"].clone()).unwrap();
    ...
    let lo = call(&app, "POST", "/api/auth/passkey/login/options", Some(json!({ "username": "alice" })), None).await;
    ...
    let lv = call(&app, "POST", "/api/auth/passkey/login/verify", Some(json!({ "ceremony_id": lcid, "credential": cred })), None).await;
    assert_eq!(lv.status, StatusCode::OK, "passwordless passkey sign-in succeeds");
    let pk_cookie = lv.cookie.unwrap();
    let me = call(&app, "GET", "/api/me", None, Some(&pk_cookie)).await;
    assert_eq!(me.body["username"], "alice", "the passkey session is a real, scoped session");
    ...
    sqlx::query("UPDATE passkeys SET sign_count = 100000").execute(&pool).await.unwrap();
    let lo2 = call(&app, "POST", "/api/auth/passkey/login/options", Some(json!({ "username": "alice" })), None).await;
    let lcid2 = lo2.body["ceremony_id"].as_str().unwrap().to_string();
    let rcr2: RequestChallengeResponse = serde_json::from_value(lo2.body["options"].clone()).unwrap();
    let cred2 = authr.do_authentication(origin, rcr2).unwrap();
    let lv2 = call(&app, "POST", "/api/auth/passkey/login/verify", Some(json!({ "ceremony_id": lcid2, "credential": cred2 })), None).await;
    assert_eq!(lv2.status, StatusCode::UNAUTHORIZED, "sign-count regression (cloned authenticator) is rejected");
}
...
async fn passkey_discoverable_login_signs_in_without_a_username_and_guards_regression() {
    ...
    use webauthn_authenticator_rs::softpasskey::SoftPasskey;
    use webauthn_authenticator_rs::WebauthnAuthenticator;
    use webauthn_rs::prelude::{Base64UrlSafeData, CreationChallengeResponse, RequestChallengeResponse, Url};
    ...
    let discoverable_assert = |authr: &mut WebauthnAuthenticator<SoftPasskey>, options: serde_json::Value| {
    ...
    let lo = call(&app, "POST", "/api/auth/passkey/discoverable/login/options", None, None).await;
    assert_eq!(lo.status, StatusCode::OK);
    assert_eq!(lo.body["options"]["mediation"], "conditional", "server forces conditional mediation");
    let lcid = lo.body["ceremony_id"].as_str().unwrap().to_string();
    let cred = discoverable_assert(&mut authr, lo.body["options"].clone());
    let lv = call(&app, "POST", "/api/auth/passkey/discoverable/login/verify", Some(json!({ "ceremony_id": lcid, "credential": cred })), None).await;
    assert_eq!(lv.status, StatusCode::OK, "discoverable passkey sign-in succeeds without a username");
    let pk_cookie = lv.cookie.unwrap();
    let me = call(&app, "GET", "/api/me", None, Some(&pk_cookie)).await;
    assert_eq!(me.body["username"], "alice", "the discoverable session is a real, scoped session");
    ...
    let last_used: Option<String> = sqlx::query("SELECT last_used_at FROM passkeys")
        ...
        .unwrap()
        .get("last_used_at");
    assert!(last_used.is_some(), "last_used_at updated via the discoverable path");

    // 3) Cloned-authenticator guard fires here too.
    sqlx::query("UPDATE passkeys SET sign_count = 100000").execute(&pool).await.unwrap();
    let lo2 = call(&app, "POST", "/api/auth/passkey/discoverable/login/options", None, None).await;
    let lcid2 = lo2.body["ceremony_id"].as_str().unwrap().to_string();
    let cred2 = discoverable_assert(&mut authr, lo2.body["options"].clone());
    let lv2 = call(&app, "POST", "/api/auth/passkey/discoverable/login/verify", Some(json!({ "ceremony_id": lcid2, "credential": cred2 })), None).await;
    assert_eq!(lv2.status, StatusCode::UNAUTHORIZED, "sign-count regression is rejected on the discoverable path too");
}
...
async fn offline_replay_of_explicit_state_mutations_is_idempotent_and_converges() {
    ...
    let read = |c: &str, v: bool| {
        let app = app.clone();
        let c = c.to_string();
        async move { call(&app, "POST", &format!("/api/items/{item}/read"), Some(json!({ "value": v })), Some(&c)).await }
    };
    ...
    assert_eq!(read(&alice_c, true).await.body["is_read"], true, "replaying read=true is a no-op");
    ...
    call(&app, "POST", &format!("/api/items/{item}/star"), Some(json!({ "value": true })), Some(&alice_c)).await;
    call(&app, "POST", &format!("/api/items/{item}/star"), Some(json!({ "value": true })), Some(&alice_c)).await;

    // Server state matches the replayed final intent.
    let a_view = call(&app, "GET", &format!("/api/items/{item}"), None, Some(&alice_c)).await;
    assert_eq!(a_view.body["is_read"], false, "converged to the last read intent");
    assert_eq!(a_view.body["is_starred"], true, "star preserved across read writes");

    // Bob's replay of his own queue is scoped to bob — alice is untouched.
    call(&app, "POST", &format!("/api/items/{item}/read"), Some(json!({ "value": true })), Some(&bob_c)).await;
    assert_eq!(call(&app, "GET", &format!("/api/items/{item}"), None, Some(&bob_c)).await.body["is_read"], true);
    assert_eq!(
        call(&app, "GET", &format!("/api/items/{item}"), None, Some(&alice_c)).await.body["is_read"],
        false,
...
async fn oauth_hidden_and_locked_down_when_unconfigured() {
    ...
    assert_eq!(
        call(&app, "GET", "/api/oauth/youtube/authorize", None, Some(&alice_c)).await.status,
        StatusCode::BAD_REQUEST,
    );
    assert_eq!(
        call(&app, "POST", "/api/oauth/reddit/sync", None, Some(&alice_c)).await.status,
        StatusCode::BAD_REQUEST,
    );
    // Auth is still required, and unknown providers 404.
    assert_eq!(call(&app, "GET", "/api/oauth/status", None, None).await.status, StatusCode::UNAUTHORIZED);
    assert_eq!(
        call(&app, "GET", "/api/oauth/myspace/authorize", None, Some(&alice_c)).await.status,
        StatusCode::NOT_FOUND,
    );
}
...
async fn oauth_reconcile_import_is_idempotent_and_per_user() {
    ...
    assert_eq!((second.added, second.skipped), (0, 3), "repeat sync adds nothing already present");

    // A partial re-sync with one extra channel adds only the new one.
    let mut more = subs.clone();
    more.push(youtube_subscription("UCnewnewnewnewnewnewnew", "New Channel"));
    let third = reconcile(&pool, &cfg, alice_id, ai, &more).await.unwrap();
    assert_eq!((third.added, third.skipped), (1, 3), "only the genuinely-new channel is added");
    ...
    assert!(list.iter().any(|f| f["kind"] == "youtube"
        && f["feed_url"] == "https://www.youtube.com/feeds/videos.xml?channel_id=UC_x5XG1OV2P6uZZ5FSM9Ttw"));
...
async fn oauth_sync_creates_feeds_that_are_not_immediately_due() {
    ...
    let row = sqlx::query(
        "SELECT next_fetch_at FROM feeds WHERE feed_url = ?",
    )
    ...
    let now_row = sqlx::query("SELECT datetime('now') AS n").fetch_one(&pool).await.unwrap();
...
async fn refresh_all_updates_only_the_caller_s_subscribed_feeds() {
    ...
    let alice_other: i64 = sqlx::query("SELECT id FROM categories WHERE user_id = ? AND name = 'Other'")
        .bind(alice_id)
        .fetch_one(&pool)
        .await
        .unwrap()
    ...
    let bob_other: i64 = sqlx::query("SELECT id FROM categories WHERE user_id = ? AND name = 'Other'")
        .bind(bob_id)
        .fetch_one(&pool)
        .await
        .unwrap()
    ...
    for (feed_id, user_id, cat) in [(feed_a1, alice_id, alice_other), (feed_a2, alice_id, alice_other), (feed_b1, bob_id, bob_other)] {
    ...
    let resp = call(&app, "POST", "/api/feeds/refresh-all", None, Some(&alice_cookie)).await;
    assert_eq!(resp.status, StatusCode::OK);
    ...
    assert_ne!(due_now(feed_a1).await, far_future, "Alice's feed 1 should now be due");
    assert_ne!(due_now(feed_a2).await, far_future, "Alice's feed 2 should now be due");
    assert_eq!(due_now(feed_b1).await, far_future, "Bob's feed must be untouched");
}
...
async fn oauth_refresh_token_is_encrypted_and_never_returned() {
    ...
    oauth::save_connection(&pool, &enc_key, alice_id, Provider::Reddit, secret, Some("read"), Some("u/alice"))
        .await
        .unwrap();

    // Stored blob is ciphertext — the plaintext token never appears at rest.
    let blob: Vec<u8> = sqlx::query("SELECT refresh_token_enc FROM user_oauth WHERE user_id = ? AND provider = 'reddit'")
        .bind(alice_id)
        .fetch_one(&pool)
        .await
        .unwrap()
    ...
    let back = oauth::load_refresh_token(&pool, &enc_key, alice_id, Provider::Reddit).await.unwrap();
    assert_eq!(back.as_deref(), Some(secret));

    // The status endpoint reports connected but never serializes the token.
    let status = call(&app, "GET", "/api/oauth/status", None, Some(&alice_c)).await;
    let reddit = status.body.as_array().unwrap().iter().find(|p| p["provider"] == "reddit").unwrap().clone();
    assert_eq!(reddit["connected"], true);
    assert_eq!(reddit["account_label"], "u/alice");
    let dump = status.body.to_string();
    assert!(!dump.contains(secret), "status response must not contain the token");
