...
pub fn parse_tz(value: Option<&str>) -> Tz {
    value.and_then(|v| Tz::from_str(v).ok()).unwrap_or(chrono_tz::UTC)
}
...
///
/// `today`/`yesterday` are calendar days in `tz`; `week`/`month` are the last 7/30 days
/// (rolling from `now`). `all` (or anything unknown) yields an unbounded range.
pub fn when_range(when: &str, tz: &Tz, now: DateTime<Utc>) -> WhenRange {
    ...
    match when {
        "today" => {
            ...
            WhenRange { start: Some(fmt(start)), end: Some(fmt(end)) }
        }
        "yesterday" => {
        ...
        "week" => WhenRange { start: Some(fmt(now - Duration::days(7))), end: None },
        "month" => WhenRange { start: Some(fmt(now - Duration::days(30))), end: None },
        _ => WhenRange::default(),
    }
}
...
mod tests {
    ...
    fn when_all_is_unbounded() {
        ...
        assert_eq!(when_range("garbage", &chrono_tz::UTC, utc("2021-06-15 12:00:00")), WhenRange::default());
    }
    ...
    fn week_and_month_are_rolling_windows() {
        ...
        assert_eq!(when_range("week", &chrono_tz::UTC, now).start.as_deref(), Some("2021-06-08 12:00:00"));
        assert_eq!(when_range("month", &chrono_tz::UTC, now).start.as_deref(), Some("2021-05-16 12:00:00"));
    ...
    fn fts_query_sanitizes_punctuation_into_prefix_terms() {
        assert_eq!(fts_query("rust async").as_deref(), Some("\"rust\"* \"async\"*"));
        ...
        assert_eq!(fts_query("  NEAR(\"x\") OR *").as_deref(), Some("\"NEARx\"* \"OR\"*"));
