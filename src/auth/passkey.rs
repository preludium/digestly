...
pub fn build(rp_id: &str, rp_origin: &str, extra_origins: &[String]) -> Option<Arc<Webauthn>> {
    ...
    for extra in extra_origins {
        match Url::parse(extra) {
            ...
            Err(e) => {
                warn!(extra_origin = extra, error = %e, "skipping invalid RP_EXTRA_ORIGINS entry")
            }
        }
    }
    match builder.build() {
...
mod tests {
    ...
    fn build_succeeds_with_extra_origins_for_split_dev_servers() {
        ...
        assert!(
            rp.is_some(),
            "RP should build successfully with a valid extra origin"
        );
    }
    ...
    fn build_ignores_an_invalid_extra_origin_without_failing() {
        ...
        assert!(
            rp.is_some(),
            "RP should still build using just the primary origin"
        );
    }
}
