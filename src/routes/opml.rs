...
async fn import(user: CurrentUser, State(state): State<AppState>, Json(body): Json<ImportBody>) -> ApiResult<Json<serde_json::Value>> {
    if let Some(items) = body.items {
        ...
        for it in items {
            let cat_id = resolve_or_create_category(&state.pool, user.id, it.category.as_deref()).await?;
            let kind = FeedKind::from_db(it.kind.as_deref().unwrap_or("rss"));
            let added = subscribe_url(&state.pool, &cfg, user.id, &it.feed_url, kind, cat_id, it.title.as_deref(), true).await?;
            if added {
                imported += 1;
            } else {
                skipped += 1;
    ...
    let Some(text) = body.opml else {
        return Err(AppError::BadRequest("provide `opml` (preview) or `items` (confirm)".into()));
    };
    let feeds = opml::parse(&text);
    if feeds.is_empty() {
        return Err(AppError::BadRequest("no feeds found in the OPML file".into()));
    }
    let mut entries = Vec::with_capacity(feeds.len());
    for f in feeds {
        let already = crate::routes::feeds::is_subscribed(&state.pool, user.id, &f.feed_url).await?;
...
async fn export(user: CurrentUser, State(state): State<AppState>) -> ApiResult<impl IntoResponse> {
    ...
    Ok((
        [
            ...
            (CONTENT_DISPOSITION, "attachment; filename=\"digestly.opml\"".to_string()),
        ],
        xml,
    ))
}
...
async fn resolve_or_create_category(pool: &SqlitePool, user_id: i64, name: Option<&str>) -> ApiResult<i64> {
    // Blank/absent or oversized (mirrors the categories endpoint's 40-char cap) → the Other catch-all.
    let target = match name.map(str::trim).filter(|s| !s.is_empty() && s.chars().count() <= 40) {
        Some(n) => n,
        None => OTHER_CATEGORY,
    };

    if let Some(row) = sqlx::query("SELECT id FROM categories WHERE user_id = ? AND name = ? COLLATE NOCASE")
        .bind(user_id)
        .bind(target)
        .fetch_optional(pool)
        .await?
