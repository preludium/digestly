...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/ai/providers/:id", axum::routing::patch(patch_provider).delete(delete_provider))
...
async fn list_providers(_admin: AdminUser, State(state): State<AppState>) -> ApiResult<Json<Vec<provider::ProviderInfo>>> {
    Ok(Json(provider::list(&state.pool).await.map_err(AppError::Internal)?))
}
...
async fn create_provider(
    ...
    if name.is_empty() || base_url.is_empty() || model.is_empty() {
        return Err(AppError::BadRequest("name, base_url and model are required".into()));
    }
...
async fn patch_provider(
    ...
    let ok = provider::patch(&state.pool, id, name, model).await.map_err(AppError::Internal)?;
    if !ok {
...
async fn activate_provider(
    ...
    let ok = provider::activate(&state.pool, id).await.map_err(AppError::Internal)?;
    if !ok {
        return Err(AppError::NotFound("provider not found".into()));
    }
    Ok(Json(serde_json::json!({ "ok": true })))
...
async fn delete_provider(
    ...
    let ok = provider::delete(&state.pool, id).await.map_err(AppError::Internal)?;
    if !ok {
        return Err(AppError::NotFound("provider not found".into()));
    }
    Ok(Json(serde_json::json!({ "ok": true })))
...
async fn test_provider(
    ...
    match llm.complete(&req).await {
        Ok(_) => Ok(Json(TestResult { ok: true, error: None })),
        Err(e) => Ok(Json(TestResult { ok: false, error: Some(e.user_message()) })),
    }
}
...
async fn get_ai_settings(_admin: AdminUser, State(state): State<AppState>) -> ApiResult<Json<AiSettingsDto>> {
    let p = ai::AiParams::load(&state.pool).await;
    let (day, month) = ai::budget::spent(&state.pool).await.map_err(AppError::Internal)?;
...
async fn put_ai_settings(
    ...
    set_setting(&state.pool, "ai.max_tokens", &body.max_tokens.clamp(64, 8192).to_string()).await?;
    set_setting(&state.pool, "ai.temperature_x100", &temp_x100.to_string()).await?;
    set_setting(&state.pool, "ai.timeout_secs", &body.timeout_secs.clamp(5, 300).to_string()).await?;
    set_setting(&state.pool, "ai.daily_token_budget", &body.daily_token_budget.max(0).to_string()).await?;
    set_setting(&state.pool, "ai.monthly_token_budget", &body.monthly_token_budget.max(0).to_string()).await?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
