...
const COMMON_PATHS: [&str; 6] = ["/feed", "/rss", "/rss.xml", "/atom.xml", "/feed.json", "/index.xml"];
...
pub async fn discover(client: &Client, cfg: &IngestSettings, input: &str) -> Result<Vec<Candidate>> {
    ...
    if let Some(sub) = reddit_subreddit(input) {
        let feed_url = url_util::normalize_url(&reddit::rss_url(&sub)).unwrap_or_else(|| reddit::rss_url(&sub));
...
async fn discover_from_url(client: &Client, cfg: &IngestSettings, url: &str) -> Result<Vec<Candidate>> {
...
async fn youtube_candidate(client: &Client, cfg: &IngestSettings, input: &str) -> Result<Option<Candidate>> {
    ...
    if let Some(id) = extract_channel_id_from_input(input) {
        return Ok(Some(youtube_from_id(&id)));
    }
...
fn youtube_from_id(id: &str) -> Candidate {
    Candidate {
        feed_url: format!("https://www.youtube.com/feeds/videos.xml?channel_id={id}"),
        title: None,
        kind: FeedKind::Youtube.as_str().to_string(),
        site_url: Some(format!("https://www.youtube.com/channel/{id}")),
        icon_url: None,
    }
}

fn looks_like_channel_id(s: &str) -> bool {
    s.len() == 24 && s.starts_with("UC") && s.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-')
}
...
async fn fetch_body(client: &Client, cfg: &IngestSettings, url: &str) -> Option<Vec<u8>> {
    match fetch::get(client, url, &Conditional { etag: None, last_modified: None }, cfg).await {
...
fn feed_candidate(body: &[u8], feed_url: &str, kind: FeedKind) -> Candidate {
    ...
    let site_url = parsed
        .as_ref()
        .and_then(|f| f.links.iter().find(|l| l.rel.as_deref() == Some("alternate")).map(|l| l.href.clone()));
    Candidate {
...
fn sniff_alternate_links(body: &[u8], base: &str) -> Vec<Candidate> {
    ...
    for el in doc.select(&sel) {
        ...
        if let Some(href) = el.value().attr("href").and_then(|h| url_util::resolve(base, h)) {
            out.push(Candidate {
...
mod tests {
    ...
    fn detects_reddit_forms() {
        ...
        assert_eq!(reddit_subreddit("/r/rust/"), Some("rust".into()));
        assert_eq!(reddit_subreddit("https://www.reddit.com/r/programming/.rss"), Some("programming".into()));
    ...
    fn detects_youtube_channel_id() {
        ...
        assert_eq!(
            extract_channel_id_from_input(&format!("https://www.youtube.com/channel/{id}")).as_deref(),
            Some(id)
        );
        assert_eq!(extract_channel_id_from_html(&format!("x\"channelId\":\"{id}\"y")).as_deref(), Some(id));
    }
    ...
    fn sniffs_alternate_links() {
        ...
        assert!(cands.iter().any(|c| c.feed_url == "https://ex.com/feed.xml"));
