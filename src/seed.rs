...
pub const STARTER_FEEDS: [(&str, &str, &str); 4] = [
    ("https://news.ycombinator.com/rss", "rss", "Software Engineering"),
    ("https://www.reddit.com/r/programming/.rss", "reddit", "Software Engineering"),
    ("https://www.reddit.com/r/MachineLearning/.rss", "reddit", "AI"),
    ("https://www.reddit.com/r/softwareengineering/.rss", "reddit", "Software Engineering"),
];
...
pub async fn seed_default_categories(pool: &SqlitePool, user_id: i64) -> Result<()> {
    for (position, name) in DEFAULT_CATEGORIES.iter().enumerate() {
        sqlx::query(
            "INSERT OR IGNORE INTO categories (user_id, name, position) VALUES (?, ?, ?)",
        )
        .bind(user_id)
        .bind(name)
