...
import type { AiPreset, AiProvider, AiSettings, NewAiProvider, TestResult } from "@/lib/types";
...
export function useCreateProvider() {
  ...
  return useMutation({
    mutationFn: (body: NewAiProvider) => api.post<{ id: number }>("/ai/providers", body),
...
export function usePatchProvider() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...body }: { id: number; name?: string; model?: string }) =>
...
export function useActivateProvider() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: number) => api.post<{ ok: boolean }>(`/ai/providers/${id}/activate`),
    onSuccess: () => qc.invalidateQueries({ queryKey: PROVIDERS_KEY }),
  });
}
...
export function useDeleteProvider() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: number) => api.del<{ ok: boolean }>(`/ai/providers/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: PROVIDERS_KEY }),
  });
}
...
export function useTestProvider() {
  return useMutation({
    mutationFn: (id: number) => api.post<TestResult>(`/ai/providers/${id}/test`),
  });
}
...
export function useUpdateAiSettings() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: Omit<AiSettings, "tokens_used_today" | "tokens_used_month">) =>
