...
import type { CategoryCounts, ItemDetail, ItemsPage, ItemState, ItemStatus, ItemType, ItemWhen, SummaryResult } from "@/lib/types";
...
export function useCategoryCounts(facets: { type: ItemType; status: ItemStatus; when: ItemWhen }) {
  ...
  return useQuery<CategoryCounts>({
    ...
    queryFn: () => api.get<CategoryCounts>(`/categories/counts${qs ? `?${qs}` : ""}`),
  });
}

/** Total unread across all feeds — the top-bar badge (§9.0). */
export function useUnreadCount(): number {
  const counts = useCategoryCounts({ type: "all", status: "unread", when: "all" });
...
function patchItemState(qc: QueryClient, id: number, state: Partial<ItemState>) {
  qc.setQueriesData<ItemsPage>({ queryKey: ITEMS_KEY }, (old) =>
    old ? { ...old, items: old.items.map((it) => (it.id === id ? { ...it, ...state } : it)) } : old,
  );
  qc.setQueryData<ItemDetail>(["item", id], (old) => (old ? { ...old, ...state } : old));
}
...
export function useSummarize() {
  ...
  return useMutation({
    mutationFn: ({ id, force }: { id: number; force?: boolean }) =>
      api.post<SummaryResult>(`/items/${id}/summarize${force ? "?force=1" : ""}`),
    onSuccess: (res, { id }) => {
