...
export function useSubscribeStarterFeeds() {
    ...
    return useMutation({
        mutationFn: () =>
            api.post<{ added: number }>("/onboarding/starter-feeds"),
        onSuccess: () => {
...
export function useUpdateIngestionSettings() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (body: IngestionSettings) =>
            api.put<IngestionSettings>("/admin/ingestion", body),
...
export function usePurgeRetention() {
    ...
    return useMutation({
        mutationFn: () =>
            api.post<{ removed: number }>("/admin/retention/purge"),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ["items"] });
            qc.invalidateQueries({ queryKey: ["feeds"] });
        },
