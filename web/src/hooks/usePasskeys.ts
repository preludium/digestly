...
export function useRegisterPasskey() {
  ...
  return useMutation({
    mutationFn: async (name?: string) => {
      const { ceremony_id, options } = await api.post<Ceremony<{ publicKey: unknown }>>(
        "/passkeys/register/options",
      );
      // `options` is a webauthn-rs CreationChallengeResponse: `{ publicKey: {...} }`.
      const credential = await runRegistration((options as { publicKey: never }).publicKey);
      return api.post<Passkey>("/passkeys/register/verify", { ceremony_id, credential, name });
    },
...
export function usePasskeyLogin() {
  ...
  return useMutation({
    mutationFn: async (username: string) => {
      ...
      const credential = await runAuthentication((options as { publicKey: never }).publicKey);
      return api.post<User>("/auth/passkey/login/verify", { ceremony_id, credential });
    },
...
export function useDiscoverablePasskeyLogin() {
  ...
  return useMutation({
    mutationFn: async (signal?: AbortSignal) => {
      ...
      return api.post<User>("/auth/passkey/discoverable/login/verify", { ceremony_id, credential });
    },
    onSuccess: (user) => qc.setQueryData(["me"], user),
  });
}
