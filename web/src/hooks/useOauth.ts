...
export function useOauthConnect() {
  return useMutation({
    mutationFn: async (provider: OAuthProvider) => {
      const { url } = await api.get<{ url: string }>(`/oauth/${provider}/authorize`);
...
export function useOauthSync() {
  ...
  return useMutation({
    mutationFn: ({ provider, categoryId }: { provider: OAuthProvider; categoryId?: number }) =>
      api.post<SyncOutcome>(`/oauth/${provider}/sync`, categoryId ? { category_id: categoryId } : undefined),
    onSuccess: () => {
...
export function useOauthDisconnect() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (provider: OAuthProvider) => api.del<{ ok: boolean }>(`/oauth/${provider}`),
