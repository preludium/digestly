...
import { coalesce, Outbox, type OutboxStore, type QueuedMutation, replay } from "./outbox";
...
describe("replay", () => {
  const retryable = (e: unknown) => e instanceof Error && e.message === "offline";
  ...
  it("sends coalesced mutations and clears the queue on success", async () => {
    ...
    const remaining = await replay(list, async (m) => void sent.push(m), retryable);
