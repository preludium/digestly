...
/** Named fetch-interval choices (§9.3/§9.4) - default is Daily, matching the ingestion default. */
...
export const DEFAULT_FETCH_INTERVAL_SECS = 86400;

/** Snap an arbitrary stored interval to the closest named option, so the Select always shows a
 *  value even for a feed whose interval predates these fixed choices. */
export function nearestFetchIntervalSecs(secs: number): number {
    return FETCH_INTERVAL_OPTIONS.reduce(
        (best, opt) =>
            Math.abs(opt.value - secs) < Math.abs(best - secs)
                ? opt.value
                : best,
        FETCH_INTERVAL_OPTIONS[0].value as number,
    );
}
...
/** Digest row heading ("Thursday, July 8"). */
...
/** Short date for a period range ("Jul 1"). */
...
/** Spelled-out relative time for the item detail view ("just now", "5 hours ago", "2 days ago",
 *  or a date once it's been over a week). */
export function relativeTimeLong(value: string | null | undefined): string {
    const d = toDate(value);
    if (!d) return "";
    const secs = Math.round((Date.now() - d.getTime()) / 1000);
    if (secs < 60) return "just now";
