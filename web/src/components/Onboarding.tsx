...
import { useSettings, useSubscribeStarterFeeds, useUpdateSettings } from "@/hooks/useSettings";
...
export function Onboarding() {
  ...
  const [tz, setTz] = useState(settings.data?.timezone && settings.data.timezone !== "UTC" ? settings.data.timezone : guessTz);
  ...
  const addStarter = () =>
    starter.mutate(undefined, {
      onSuccess: (r) => {
        ...
        toast(`Added ${r.added} starter feed${r.added === 1 ? "" : "s"}`, "success");
      },
      onError: (e) => toast(e instanceof Error ? e.message : "Could not add feeds", "error"),
    });

  const finish = (withTz: boolean) => {
    const body = withTz ? { timezone: tz, onboarded: true } : { onboarded: true };
    update.mutate(body, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/90 p-4">
      <div className="w-full max-w-md space-y-5 rounded-lg border border-border bg-card p-6 shadow-xl">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="ob-tz" value={tz} onChange={(e) => setTz(e.target.value)} placeholder="Europe/Warsaw" />
          <p className="text-xs text-muted-foreground">Used for “Today/This week” filters and the digest schedule.</p>
        </div>
        ...
        {showPasskey && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Add a passkey now for passwordless sign-in — or later from your profile.
            </p>
        ...
        <p className="text-xs text-muted-foreground">
          Tip: set up push notifications and (for admins) an AI provider under Settings.
        </p>

        <div className="flex justify-end gap-2 pt-1">
          <Button variant="ghost" disabled={update.isPending} onClick={() => finish(false)}>
            Skip
          </Button>
          <Button disabled={update.isPending} onClick={() => finish(true)}>
