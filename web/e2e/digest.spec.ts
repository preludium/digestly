...
import { FIXTURE, registerUser, seedDigest } from "./support/api";
...
test("admin digest run produces a viewable digest", async ({ page }) => {
    ...
    // Subscribes the fixture RSS feed, ingests, and runs the admin digest in a SEPARATE request
    // context so the admin hf_session never clobbers the page user's session (see cookie contract
    // in support/api.ts). With no AI provider configured the backend falls back to raw grouped
    // items (digest/mod.rs:195-198,308-331), which produces a non-empty archived digest.
    await seedDigest(page.request);
