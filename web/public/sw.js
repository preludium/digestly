...
self.addEventListener("install", (event) => {
  event.waitUntil(caches.open(SHELL_CACHE).then((c) => c.addAll(SHELL_URLS)).catch(() => {}));
});
...
async function notifyClientsToFlush() {
  ...
  for (const client of clients) client.postMessage({ type: "hf-flush-outbox" });
}
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname.startsWith("/api/")) {
    ...
    if (url.pathname === "/api/health" || url.pathname.startsWith("/api/auth")) return;
    event.respondWith(
      (async () => {
        try {
