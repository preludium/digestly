...
(`docs/s2-android-plan.md`) that reuses 100% of the web app and **avoids the server auth change**,
...
auth, service worker, offline reading, and the S3 write-sync outbox all work **unchanged — no
server changes at all**. The same `web/dist` the server already serves is what the app renders, so
there is no second bundle and no UI fork. Native features (splash, status bar, back button, local
...
server up to *open* the app. After first load, the service worker still serves cached items for
**reading** offline, and the S3 outbox still queues writes. If cold-start-offline matters, use the
bundled+bearer path instead (or the installed PWA, which does cache its shell). For a home reader,
"online to launch" is usually acceptable.
...
- **Passkeys (S1): risky in a raw WebView.** WebAuthn support in Android's *System WebView*
  (what Capacitor uses) is inconsistent, unlike full Chrome (what a TWA uses). Passkey login may
  not work in the Capacitor WebView. Mitigations: rely on password login in the app, and/or add a
...
const config: CapacitorConfig = {
  ...
  server: {
    ...
    androidScheme: "https",
    allowNavigation: ["digestly.<tailnet>.ts.net"], // keep the app on your origin
    cleartext: false,
  },
};
...
- **Debug APK:** Android Studio → *Build > Build APK*, or `cd android && ./gradlew assembleDebug`.
- **Signed release:** create an upload keystore (`keytool`), add `signingConfigs` to
  `android/app/build.gradle` (or use Android Studio's *Generate Signed Bundle/APK*) → signed
...
1. **Phase 1 — minimal wrapper (~0.5–1 day on a prepared machine).** Add deps + `capacitor.config.ts`
   ...
   That already satisfies the S2 gate's spirit (installs, points at server, logs in, reads).
...
- **Needs an Android machine + device (NOT here):** `cap add android`, the Gradle/APK build,
  install, and the on-device login/read smoke test — same environment limit as any Android target.
...
If you want, I can do **Phase 1 prep now** (config + guarded `native.ts` + deps, verify the web
build stays green) so the project is ready to `cap add android` on a machine that has the Android
SDK — without touching the server or the existing UI behaviour.
