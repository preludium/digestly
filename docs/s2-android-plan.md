...
and the decisions to make before starting. Authoritative spec: `prompt.md` §1 (mobile), §9a
(Tauri), §14.3 (mobile README); stretch summary in `phases/phase-S-stretch.md`.
...
1. **Auth can't be cookie-based cross-origin.** `SameSite=Lax` cookies are **not sent** on
   ...
   won't travel from the Tauri origin to the server. §1a anticipates this: "issue a signed
   token/cookie … that works for the PWA (cookie/localStorage) and Tauri Android (Keystore)."
   → We need **bearer-token auth** in addition to the cookie.
...
The token is just the existing opaque **session id** (from the `sessions` table), delivered via
...
- **First-launch config screen** (§9a): if running in Tauri and no base URL is set, show a screen
  to enter the server URL (validate it's https, **reject localhost** per spec), ping `/api/health`,
  then continue to login. Editable later in Settings.
...
- **Scaffold** a Tauri v2 project pointing `frontendDist` at `web/dist`; `tauri.conf.json` with the
  app id (e.g. `net.ts.digestly`), name, icons, and Android config.
...
| Robust offline base         | Ships the S3 outbox + S3/PWA caching inside a controlled WebView.               |
...
| **Heavy toolchain**                           | JDK + Android SDK + NDK + Rust Android targets + Tauri CLI. Multi-GB; not installed here; finicky version matching (NDK ↔ Gradle ↔ Tauri).                                             |
...
| **Marginal value vs. PWA**                    | The installed PWA already gives home-screen install + offline reading + queued writes. Tauri's _net new_ value is Keystore + biometric + native notifications - real, but incremental. |
...
standalone window, offline reading, and the S3 write-sync outbox - **zero** additional code,
...
1. **Phase A - server + client auth/base-URL (no Tauri yet).** Bearer-token auth in `CurrentUser`,
   ...
   and de-risks the hardest part first - it even lets a browser PWA point at a remote origin.
